...
type State =
  ...
  | { status: "ready"; live: ScrollsMoment[] };
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        ...
        if (!cancelled) setState({ status: "ready", live });
      } catch (err) {
  ...
  return (
    <div>
      ...
      <section className="mx-auto max-w-3xl px-5 pb-16">
        ...
        {state.status === "ready" && state.live.length === 0 ? (
          <p className="text-white/55">No one is live right now. Check back soon.</p>
        ) : null}

        {state.status === "ready" && state.live.length > 0 ? (
          <div className="grid gap-4 sm:grid-cols-2">
            {state.live.map((moment) => {
              const user = moment.user;
              const name = user?.displayName ?? user?.display_name ?? user?.username ?? "Scrolls";
              return (
                <Link
                  key={moment.liveSession!.id}
